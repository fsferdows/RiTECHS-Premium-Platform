...
export default function TrainingSection() {
  ...
  return (
    <section id="training-section" className="py-16 bg-white border-b border-divider-gold/40">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16 items-end">
          <div>
            ...
            <h2 className="font-serif-display text-3xl sm:text-4.5xl text-primary-navy font-bold">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-neutral-600 leading-relaxed font-light mt-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col gap-3">
            <span className="text-[10px] font-mono uppercase tracking-widest text-[#6B7280] font-bold block mb-2">
            ...
            {courses.map((course) => (
              <button
                ...
                className={`p-5 text-left border rounded-xs transition-all flex flex-col gap-2 relative ${
                  selectedCourse === course.id
                    ? 'border-accent-gold bg-neutral-warm/20 shadow-xs'
                    : 'border-divider-gold/25 bg-white hover:border-accent-gold/50'
                }`}
              >
                {selectedCourse === course.id && (
                ...
                <h4 className="font-serif-display text-sm font-bold text-primary-navy leading-snug">
                  {course.title}
                </h4>
                <div className="text-[11px] text-muted-gray">By {course.instructor}</div>
              </button>
            ))}
          </div>

          {/* Core Selected course Details Pane */}
          <div className="lg:col-span-7 bg-neutral-warm/20 border border-divider-gold/30 p-8 rounded-sm relative">
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <h3 className="font-serif-display text-xl sm:text-2xl font-bold text-primary-navy mt-1">
                  {activeCourse.title}
                </h3>
              </div>
            ...
            <div className="bg-white border border-divider-gold/15 p-5 mb-6 rounded-xs">
              <span className="font-mono text-[9px] text-[#52525B] font-bold uppercase tracking-wider block mb-3">
                Accredited Syllabus Chapters
              </span>
              <div className="flex flex-col gap-2.5">
                {activeCourse.syllabus.map((chapter, idx) => (
                  <div key={idx} className="flex gap-2.5 items-start text-xs text-primary-navy/90 font-medium">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-divider-gold/15 pt-6">
              <div>
                <p className="text-[10px] font-mono text-muted-gray uppercase">Sponsor Vetting Authority</p>
                <div className="font-serif-display text-xs font-bold text-primary-navy">
              ...
              <button
                ...
                className="bg-accent-gold hover:bg-[#B3934B] text-primary-navy py-3 px-6 text-xs uppercase tracking-widest font-sans font-bold transition-all shrink-0 rounded-xs"
              >
                Reserve Seat Now
              </button>
            </div>
            ...
            {isReserved && (
              <div className="absolute inset-0 bg-primary-navy/95 backdrop-blur-xs flex flex-col items-center justify-center p-6 text-center animate-fade-in text-white rounded-sm">
                ...
                <button
                  ...
                  className="text-xs font-mono text-accent-gold select-none font-bold uppercase tracking-wider underline hover:text-white transition-colors"
                >
                  Close panel
                </button>
              </div>
