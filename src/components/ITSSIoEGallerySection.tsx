...
export default function ITSSIoEGallerySection() {
  ...
  return (
    <section id="itss-ioe-2021-gallery" className="py-16 bg-maroon-dark text-white border-b border-accent-gold/15 premium-noise">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 text-left">
          <div>
            ...
            <p className="text-xs sm:text-sm text-neutral-300 leading-relaxed font-light max-w-xl mt-2">
          ...
          <div className="flex flex-wrap gap-2 select-none">
            {[
              ...
              <button
                ...
                className={`py-2 px-3 text-[10px] font-mono uppercase tracking-wider rounded-xs border transition-all cursor-pointer ${
                  activeFilter === filter.id
                    ? 'bg-accent-gold border-accent-gold text-primary-maroon font-bold shadow-md'
                    : 'bg-maroon-dark border-accent-gold/20 text-neutral-300 hover:text-white'
                }`}
              >
                {filter.label}
              </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredItems.map((item) => (
            <div
              ...
              className="group cursor-pointer bg-primary-maroon border border-accent-gold/25 rounded-xs overflow-hidden hover:border-accent-gold transition-all duration-300 shadow-xl text-white text-left"
            >
              ...
              <div className="relative aspect-[4/3] w-full overflow-hidden bg-black/20">
                ...
                />
                <div className="absolute inset-0 bg-primary-maroon/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                ...
                <div className="absolute top-3 left-3 bg-maroon-dark/95 border border-accent-gold/35 px-2.5 py-1 text-[9px] font-mono text-accent-gold uppercase tracking-widest font-semibold rounded-xs">
                  {item.categoryLabel}
                </div>
              </div>
              ...
              <div className="p-5 flex flex-col gap-2.5">
                <div className="flex items-center gap-1.5 text-[9px] font-mono text-neutral-300">
                  <Calendar className="w-3 h-3 text-accent-gold" />
                  <span>{item.date}</span>
                </div>
                <h4 className="font-serif-display text-sm font-bold text-white leading-snug group-hover:text-accent-gold transition-colors">
                  {item.title}
                </h4>
                <p className="text-[11px] text-neutral-300 leading-relaxed font-light line-clamp-2">
                  {item.desc}
                </p>
                <div className="flex items-center gap-1 text-[10px] text-neutral-300 border-t border-accent-gold/15 pt-3 mt-1.5">
        ...
        {selectedItem && (
          <div className="fixed inset-0 bg-[#030a17]/95 backdrop-blur-md z-55 flex items-center justify-center p-6 animate-fade-in select-text">
            <button
              onClick={() => setSelectedItem(null)}
              className="absolute top-6 right-6 p-2 rounded-full border border-white/20 hover:border-accent-gold text-white hover:text-accent-gold transition-all cursor-pointer"
              title="Close Portal"
            >
              <X className="w-6 h-6" />
            </button>

            <div className="bg-primary-maroon border border-accent-gold/30 max-w-4xl w-full rounded-sm overflow-hidden grid grid-cols-1 md:grid-cols-12 max-h-[90vh] shadow-2xl relative">
              ...
              {/* Context column */}
              <div className="md:col-span-5 p-8 flex flex-col justify-between bg-primary-maroon relative text-white text-left">
                <div className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-1 text-[10px] font-mono text-neutral-300 mt-2">
                  ...
                  <div className="h-px bg-accent-gold/20 my-1" />
                ...
                <div className="pt-6 border-t border-accent-gold/20 flex items-center gap-2 text-xs font-mono text-accent-gold font-bold">
