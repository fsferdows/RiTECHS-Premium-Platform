...
export default function AcademicModificationSection() {
  ...
  return (
    <section id="academic-modification" className="py-20 bg-white border-b border-divider-gold/40">
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            <p className="text-xs sm:text-sm text-primary-navy leading-relaxed font-medium">
            ...
            {/* Quick trust metrics */}
            <div className="grid grid-cols-3 gap-4 border-t border-divider-gold/20 pt-6 mt-2">
              <div>
                <div className="font-serif-display text-xl font-bold text-accent-gold">100%</div>
                <div className="text-[10px] font-mono uppercase tracking-wider text-muted-gray">Academic Rigor</div>
              </div>
              <div>
                <div className="font-serif-display text-xl font-bold text-accent-gold">NDA</div>
                <div className="text-[10px] font-mono uppercase tracking-wider text-muted-gray">IP protection</div>
              </div>
              <div>
                <div className="font-serif-display text-xl font-bold text-accent-gold">Q1</div>
                <div className="text-[10px] font-mono uppercase tracking-wider text-muted-gray">Journal-Ready</div>
              </div>
            </div>
          </div>

          {/* Column 2: Interactive Tabs & Content */}
          <div className="lg:col-span-7 bg-neutral-warm/25 border border-divider-gold/30 p-8 rounded-sm relative overflow-hidden">
            ...
            {/* Tab Swappers */}
            <div className="flex border-b border-divider-gold/20 mb-8 overflow-x-auto gap-2">
              {[
                ...
                return (
                  <button
                    ...
                    className={`py-3.5 px-4 text-xs font-mono uppercase tracking-widest flex items-center gap-2 border-b-2 transition-all duration-300 ${
                      activeTab === tab.id
                        ? 'border-accent-gold text-accent-gold font-bold bg-white/50'
                        : 'border-transparent text-muted-gray hover:text-primary-navy'
                    }`}
                  >
            ...
            <div>
              {activeTab === 'editing' && (
                <div className="animate-fade-in flex flex-col gap-6">
                  <div className="flex flex-col gap-2">
                    <h3 className="font-serif-display text-xl text-primary-navy font-bold">Academic Editing Service</h3>
                    <p className="text-xs sm:text-sm text-muted-gray leading-relaxed font-light">
                  ...
                  <div className="bg-white/60 p-4 border border-divider-gold/15 rounded-xs flex flex-col gap-2">
                    <span className="font-mono text-[9px] text-accent-gold font-bold">CORE DELIVERABLES</span>
                    <ul className="text-xs text-primary-navy/80 space-y-1.5 list-none font-sans font-medium">
              ...
              {activeTab === 'proofreading' && (
                <div className="animate-fade-in flex flex-col gap-6">
                  <div className="flex flex-col gap-2">
                    <h3 className="font-serif-display text-xl text-primary-navy font-bold">Academic Proofreading</h3>
                    <p className="text-xs sm:text-sm text-muted-gray leading-relaxed font-light">
              ...
              {activeTab === 'formatting' && (
                <div className="animate-fade-in flex flex-col gap-6">
                  <div className="flex flex-col gap-2">
                    <h3 className="font-serif-display text-xl text-primary-navy font-bold">Academic Formatting</h3>
                    <p className="text-xs sm:text-sm text-muted-gray leading-relaxed font-light">
              ...
              <div className="mt-8 border-t border-divider-gold/20 pt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 border shadow-2xs">
                <div>
                  ...
                  <div className="font-serif-display text-base font-bold text-primary-navy">
                    {contacts[activeTab].name}
                  </div>
                  <p className="text-[11px] text-muted-gray">RiTECHS Senior Vetting Scholar</p>
                </div>
                
                <div className="flex flex-col gap-1.5 text-xs font-mono shrink-0">
                  <a 
                    href={`tel:${contacts[activeTab].phone.replace(/[^0-9+]/g, '')}`} 
                    className="flex items-center gap-2 text-[#52525B] hover:text-[#C9A961] transition-colors"
                  >
